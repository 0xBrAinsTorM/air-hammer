...
def connect_to_wifi(ssid, password, username, interface, supplicant, outfile=None, authentication="wpa-enterprise"):
...
parser.add_argument('-s', type=int, default=0, dest='start', metavar='line',
                    help='Optional start line to resume the attack. May not be used with a password list.')
...
if (args.password is None) and (args.passfile is None):
    print("You must specify a password or a password list.")
    sys.exit()
...
if passfile is not None:
    ...
    # If there is a trailing line at the end of the file, remove it from the password list
    if passwords[-1] == "":
...
# Read usernames into an array, users
